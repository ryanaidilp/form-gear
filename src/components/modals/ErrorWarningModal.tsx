          ...
          <Show when={hasMultiplePages()}>
            <div class="flex justify-start items-center text-center font-light px-3 pb-3">
              <Show when={!isFirstPage()}>
                <button type="button" class="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-light text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm"
                  onClick={() => props.onPageChange(props.currentPage - 1)}>Prev</button>
              </Show>
              <div class="text-center px-4 text-xs">{props.currentPage} / {props.maxPage}</div>
              <Show when={!isLastPage()}>
                <button type="button" class="w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-light text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:w-auto sm:text-sm"
                  onClick={() => props.onPageChange(props.currentPage + 1)}>Next</button>
              </Show>
            </div>
          </Show>
        </div>
