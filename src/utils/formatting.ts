 ...
 *
 * @param arr - Array of numbers (or values convertible to numbers)
 * @returns The sum
 *
 * @example
 * ```typescript
...
export function sum(arr: unknown[]): number {
  return arr.reduce(
    (total: number, item) => Number(total) + Number(item),
    0
  ) as number;
}

/**
 * Finds a combination of numbers from a list that sum to a target.
 *
 * Used for checkbox value decomposition.
 *
 * @param target - Target number to sum to
 * @param listNumbers - Available numbers to use
 * @returns Array of numbers that sum to target, or empty if not possible
 *
 * @example
 * ```typescript
 * findSumCombination(7, [1, 2, 4]) // [4, 2, 1]
 * findSumCombination(5, [1, 2, 4]) // [4, 1]
 * findSumCombination(3, [2, 4]) // [] (not possible)
 * ```
 */
export function findSumCombination(
  target: number,
  listNumbers: number[]
): number[] {
  const result: number[] = [];
  const sorted = [...listNumbers].sort((a, b) => b - a); // Sort descending

  // Quick check: is target in the list?
  if (listNumbers.includes(target)) {
    return [target];
  }
  ...
  // Return empty if we couldn't reach exactly the target
...
/**
 * Transforms checkbox options to include power-of-2 values.
 *
 * Used for encoding multiple selections as a single number.
 *
 * @param options - Array of option objects
 * @returns Options with checkboxValue property added
 *
 * @example
 * ```typescript
 ...
 * // Returns:
 * // [
 * //   { label: 'A', value: 'a', checkboxValue: 1 },
 * //   { label: 'B', value: 'b', checkboxValue: 2 },
 * //   { label: 'C', value: 'c', checkboxValue: 4 },
 * // ]
 * ```
 */
export function transformCheckboxOptions<T extends Record<string, unknown>>(
  options: T[]
): Array<T & { checkboxValue: number }> {
